 ...
 * Naming: Use 'is' prefix for boolean variables for clarity
 */
const initialState = {
  ...
  // UI state
  isLoading: false, // ✅ Boolean prefix for clarity
  error: null,
  isAutoRefreshPaused: false, // ✅ Boolean prefix, circuit breaker state
};
...
export const selectIsLoading = (state) => state.campaigns.isLoading; // ✅ Updated name

export const selectError = (state) => state.campaigns.error;

export const selectIsAutoRefreshPaused = (state) => state.campaigns.isAutoRefreshPaused; // ✅ Updated name

/**
 * Memoized selector - get selected campaign object
 * Performance: Only recalculates when list or selectedId changes
 */
...
/**
 * Memoized selector - Calculate Total CTR
...
/**
 * Memoized selector - Calculate Recent CTR
 * Uses only the most recent API response
 * Performance: Only recalculates when recent data changes
 */
export const selectRecentCTR = createSelector(
  [(state) => state.campaigns.dashboardData.recent],
  (recent) => {
    if (!recent) {
      return '0.00';
    }
...
describe('campaignsSlice reducers', () => {
  ...
  test('selectCampaign clears error state', () => {
    ...
    expect(state.isAutoRefreshPaused).toBe(false);
  });

  test('pauseAutoRefresh sets isAutoRefreshPaused to true', () => {
    const action = pauseAutoRefresh();
    const state = campaignsReducer(initialState, action);
    
    expect(state.isAutoRefreshPaused).toBe(true);
  });

  test('resumeAutoRefresh resets error and unpauses', () => {
    const stateWithError = {
      ...initialState,
