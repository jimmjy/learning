...
import {
  ...
  pauseAutoRefresh,
  resumeAutoRefresh,
  resetCampaignData, // ✅ ADD THIS IMPORT
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    if (dashboardData) {
      iterationRef.current = dashboardData.iteration;
    }
  ...
  useEffect(() => {
    ...
    // ✅ RESET campaign data (iteration back to 0)
  ...
  useEffect(() => {
    ...
    // Only fetch if iteration is 0 (just reset)
    if (dashboardData.iteration === 0) {
      dispatch(
        loadMetrics({
          campaignId,
          iteration: 0,
  ...
  }, [campaign, dashboardData, dispatch, id]);
  ...
  }, [campaign, dashboardData, id, dispatch]);
